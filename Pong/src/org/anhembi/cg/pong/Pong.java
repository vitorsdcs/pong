...
public class Pong extends GLCanvas implements GLEventListener, KeyListener{
	...
	/**
	 * Exibe qual fase o jogador está em forma de texto.
	 */
	...
	/**
	 * Exibe a bola e ativa sua movimentação.
	 */
	...
	/**
	 * Controla toda a movimentação da bola,
	 * inclusive os caminhos que a mesma toma após se colidir.
	 */
		...
		if (colisao != 'F') {
			switch (colisao) {
				...
				case 'B':
					...
					break;
				case 'O':
					// Colisão no topo ou base do objeto.
	...
	/**
	 * Dispara ações de avanço de fase e perda de vida que ocorrem nas colisões.
	 ...
	 * Retorna:
	 * X para colisão nas laterais;
	 * Y para colisão no topo/base;
	 * B para colisão no bastão;
	 * O para colisão no objeto central.
	 * F para nenhuma colisão.
	 */
	private char objetoColisao() {
		// Colisão nas laterais.
		if (bolaX >= 1.0f || bolaX <= -1.0f) return 'X';
		// Colisão no topo/base.
